...
.factory('SyncSDB', function($rootScope) {
	...
	CreateSync.prototype.sendData = function(table, toInsert, toUpdate, toDelete, callback) {
		...
		for (i = 0, l = toInsert.length; i < l; i++) {
			if (toSave.indexOfKey('id', toInsert[i].id) < 0) {
				toSave.put(toInsert[i]);
			}
		}

		_putItems.call(this, table, toSave, deleteFn, callback);

		function deleteFn() {
			_deleteItems.call(this, table, toDelete, callback, callback);
		}
	}
	
	CreateSync.prototype.getData = function(table, callback) {
		...
		_getData.call(this, sql, function(data) {
			callback(null, data, []);
		}, callback);
	}
